...
var Carousel = (0, _createReactClass2.default)({
  ...
  getTouches: function getTouches(e) {
    ...
    if (isPortrait) {
      var _ref = [touches[0].pageY, -touches[0].pageX];
      touches[0].pageX = _ref[0];
      touches[0].pageY = _ref[1];
    }
    return touches;
  },
  getTouchEvents: function getTouchEvents() {
    ...
    return {
      ...
      onTouchMove: function onTouchMove(e) {
        var touches = self.getTouches(e);
        var direction = self.swipeDirection(self.touchObject.startX, touches[0].pageX, self.touchObject.startY, touches[0].pageY);

        if (direction !== 0) {
          e.preventDefault();
        }

        var length = self.props.vertical ? Math.round(Math.sqrt(Math.pow(touches[0].pageY - self.touchObject.startY, 2))) : Math.round(Math.sqrt(Math.pow(touches[0].pageX - self.touchObject.startX, 2)));
